...
library(RHMS)
data1 <- read.table("https://raw.githubusercontent.com/hydrocodes/Varios/master/22_input.txt", header=TRUE)  #leer hidrograma de entrada
data2 <- read.table("https://raw.githubusercontent.com/hydrocodes/Varios/master/22_sc.txt", header=TRUE) #leer datos elevacion m vs almacenamiento MMC
data3 <- read.table("https://raw.githubusercontent.com/hydrocodes/Varios/master/22_dc.txt", header=TRUE) #leer datos elevacion m vs caudal por vertedero m3/s
...
plot(reservoir_sim$operation[,1],typ="o", ylab="Discharge rate (m3/s)", xlab="Time step")
